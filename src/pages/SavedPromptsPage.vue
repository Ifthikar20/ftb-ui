<template>
  <div class="sp-page">
    <header class="sp-header">
      <div>
        <h1 class="sp-title">Saved Prompts</h1>
        <p class="sp-sub">
          Prompts you've added to {{ activeWebsiteName || 'this website' }}. Run an
          audit on the LLM Ranking page to test how Claude, GPT-4, Gemini, and
          Perplexity answer them.
        </p>
      </div>
      <div class="sp-head-actions">
        ...
        >
          Run audit on saved prompts
        </AirButton>
      </div>
    </header>

    <!-- Stat tiles -->
    <div v-if="prompts.length" class="sp-stat-grid">
      <div class="sp-stat-tile">
        <div class="sp-stat-icon" style="background: rgba(91, 141, 239, 0.12); color: var(--brand-accent)">
        ...
        <div>
          <div class="sp-stat-label">Total saved</div>
          <div class="sp-stat-val">{{ prompts.length }}</div>
        </div>
      </div>
      <div class="sp-stat-tile">
        <div class="sp-stat-icon" style="background: rgba(16, 185, 129, 0.12); color: var(--color-success, #059669)">
        ...
        <div>
          <div class="sp-stat-label">Tested</div>
          <div class="sp-stat-val">{{ testedCount }}</div>
        </div>
      </div>
      <div class="sp-stat-tile">
        <div class="sp-stat-icon" style="background: rgba(180, 83, 9, 0.10); color: var(--color-warning, #b45309)">
        ...
        <div>
          <div class="sp-stat-label">Untested</div>
          <div class="sp-stat-val">{{ untestedCount }}</div>
        </div>
      </div>
      <div class="sp-stat-tile">
        <div class="sp-stat-icon" style="background: rgba(126, 34, 206, 0.10); color: #7e22ce">
        ...
        <div>
          <div class="sp-stat-label">Avg trend</div>
          <div class="sp-stat-val">{{ avgTrend }}</div>
        </div>
      </div>
    </div>

    <!-- Filters -->
    <div v-if="prompts.length" class="sp-filters">
      <input
        v-model="search"
        type="search"
        class="sp-search"
      ...
      <label class="sp-select-wrap">
        <span class="sp-select-label">Style</span>
        <select v-model="styleFilter" class="sp-select">
          <option value="">All</option>
          <option v-for="s in styleOptions" :key="s.value" :value="s.value">{{ s.label }}</option>
        </select>
      </label>
      <label class="sp-select-wrap">
        <span class="sp-select-label">Source</span>
    ...
    <AirCard size="md" :padded="false">
      <div v-if="loading" class="sp-state">Loading saved prompts…</div>
      <div v-else-if="!prompts.length" class="sp-empty">
        ...
        <div class="sp-empty-title">No saved prompts yet</div>
        <p class="sp-empty-sub">
          Search for prompts that match your brand and click <strong>Save</strong>
          to add them here. Once you have a few saved, run an audit on the
          LLM Ranking page to start measuring visibility.
        </p>
        <AirButton
          variant="primary"
          size="md"
      ...
      <div v-else class="overflow-x-auto">
        <table class="sp-table">
          ...
          <tbody>
            <tr v-for="row in filteredPrompts" :key="row.id" class="sp-row">
              <td><span class="sp-id-pill">{{ formatId(row.id) }}</span></td>
              <td><AirChip size="xs" variant="neutral">{{ titleCase(row.style) }}</AirChip></td>
              <td class="sp-prompt-cell">
                <div class="sp-prompt-text">{{ row.text || row.template_text }}</div>
              </td>
              <td><AirChip size="xs" variant="info">{{ row.source || 'manual' }}</AirChip></td>
              <td class="sp-muted">{{ row.last_tested_at ? formatRelative(row.last_tested_at) : '—' }}</td>
              <td>
                <span v-if="!row.runs_count" class="sp-status sp-status-untested">Untested</span>
                <span v-else-if="(row.effectiveness_score || 0) >= 0.7" class="sp-status sp-status-good">Top</span>
                <span v-else-if="(row.effectiveness_score || 0) >= 0.4" class="sp-status sp-status-mid">Steady</span>
                <span v-else class="sp-status sp-status-low">Underperforming</span>
              </td>
              <td class="text-right">
                <button
                  class="sp-action-btn"
                  ...
                  @click="onTest(row)"
                  :title="`Test against Claude (1 provider smoke test)`"
                >
                  <span v-if="testingId === row.id" class="sp-spinner"></span>
                  <span v-else>Test</span>
                </button>
                <button
                  class="sp-action-btn sp-action-btn-ghost"
                  @click="onRemove(row)"
            ...
            <tr v-if="!filteredPrompts.length">
              <td colspan="7" class="sp-empty-filter">No prompts match these filters.</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <BaseModal v-model="smokeOpen" title="Test result" subtitle="Single-provider smoke test" wide>
      ...
      <div v-else class="sp-state">Running…</div>
    </BaseModal>
  </div>
</template>
...
const websiteId = computed(() => route.params.websiteId || appStore.activeWebsite?.id || '')
const activeWebsiteName = computed(() => appStore.activeWebsite?.name || '')
...
.sp-head-actions { display: flex; align-items: center; gap: 10px; }
...
.sp-stat-label { font-size: 12px; color: var(--text-secondary); margin: 0; }
.sp-stat-val { font-size: 22px; font-weight: 600; color: var(--text-primary); margin-top: 2px; font-variant-numeric: tabular-nums; }
...
.sp-search:focus-visible { border-color: var(--brand-accent); box-shadow: 0 0 0 3px var(--brand-accent-glow); }
.sp-select-wrap { display: inline-flex; align-items: center; gap: 8px; }
.sp-select-label { font-size: 11px; text-transform: uppercase; letter-spacing: 0.06em; color: var(--text-muted); }
.sp-select {
...
.sp-row:hover td { background: var(--bg-surface, rgba(0, 0, 0, 0.02)); }
.sp-prompt-cell { max-width: 56rem; }
.sp-prompt-text {
...
.sp-muted { color: var(--text-muted); font-size: 13px; }
...
.sp-status-untested { background: rgba(180, 83, 9, 0.10); color: var(--color-warning, #b45309); }
.sp-status-good { background: rgba(16, 185, 129, 0.12); color: var(--color-success, #059669); }
.sp-status-mid { background: rgba(91, 141, 239, 0.12); color: var(--brand-accent); }
.sp-status-low { background: rgba(220, 38, 38, 0.10); color: #b91c1c; }
...
.sp-action-btn:hover:not(:disabled) { filter: brightness(0.96); }
...
.sp-action-btn-ghost:hover { background: var(--bg-surface, rgba(0, 0, 0, 0.04)); color: var(--color-danger, #dc2626); }
.sp-spinner {
...
.sp-empty-title { font-size: 16px; font-weight: 600; color: var(--text-primary); }
.sp-empty-sub { font-size: 14px; color: var(--text-muted); max-width: 32rem; line-height: 1.55; margin: 0 0 12px; }
</style>
